...
# ── Load icons SETELAH pygame.init ────────────────────────────────────────────
icons.load_icons()
...
# ── Backsound loop ────────────────────────────────────────────────────────────
_music_loaded = False
for snd_name in ["soundtrack.wav","soundtrack.mp3"]:
...
if not _music_loaded:
    print("[audio] Tidak ada file soundtrack — game tetap berjalan tanpa musik.")
...
def add_extra_connections(grid, cols, rows, count=8):
    ...
    while added<count and attempts<600:
        attempts+=1
        x=random.randrange(2,cols-1); y=random.randrange(2,rows-1)
        if grid[y][x]==1:
            nb=sum(1 for dx,dy in [(0,1),(0,-1),(1,0),(-1,0)]
                   if 0<=x+dx<cols and 0<=y+dy<rows and grid[y+dy][x+dx]==0)
            if nb>=2:
                grid[y][x]=0; added+=1
...
class Particle:
    def __init__(self,x,y,color):
        self.x,self.y=x,y
        self.vx=random.uniform(-4,4); self.vy=random.uniform(-6,-0.5)
        self.color=color; self.life=random.randint(18,48)
        self.max_life=self.life; self.r=random.randint(2,6)
    def update(self):
...
class Player:
    def __init__(self,x,y):
        self.gx,self.gy=x,y
        self.px=MAZE_X+x*TILE+TILE//2; self.py=MAZE_Y+y*TILE+TILE//2
        self.tx,self.ty=self.px,self.py
        self.moving=False; self.speed=4
        self.lives=3; self.score=0; self.invincible=0
        self.direction=1; self.step_sfx_t=0
    ...
    def try_move(self,dx,dy,grid):
        ...
        if 0<=nx<COLS and 0<=ny<ROWS and grid[ny][nx]==0:
            ...
            self.tx=MAZE_X+nx*TILE+TILE//2; self.ty=MAZE_Y+ny*TILE+TILE//2
            self.moving=True
            if dx>0: self.direction=1
            if dx<0: self.direction=-1
            self.step_sfx_t-=1
            if self.step_sfx_t<=0:
                play_sfx(sfx_footstep,0.25); self.step_sfx_t=8

    def update(self):
        if self.moving:
            dx=self.tx-self.px; dy=self.ty-self.py; dist=math.hypot(dx,dy)
            if dist<=self.speed: self.px,self.py=self.tx,self.ty; self.moving=False
            else: self.px+=dx/dist*self.speed; self.py+=dy/dist*self.speed
        if self.invincible>0: self.invincible-=1

    def draw(self,surf):
        blink=self.invincible>0 and (self.invincible//5)%2==0
        if blink: return
        cx,cy=int(self.px),int(self.py); r=TILE//2-3
        # Coba gambar dengan ikon thief PNG
        thief_img = icons.get("thief", TILE-4)
        if thief_img:
            if self.direction==-1:
                thief_img=pygame.transform.flip(thief_img,True,False)
            surf.blit(thief_img,(cx-thief_img.get_width()//2, cy-thief_img.get_height()//2))
        else:
            # Fallback vector
        ...
        for i in range(max(0,self.lives)):
            ...
            pygame.draw.circle(surf,RED,(hx+2,cy-r-9),3)
            pygame.draw.polygon(surf,RED,[(hx-5,cy-r-7),(hx+5,cy-r-7),(hx,cy-r-2)])
...
class Guard:
    def __init__(self,x,y,speed_mult=1.0,detect_range=5):
        ...
        self.px=MAZE_X+x*TILE+TILE//2; self.py=MAZE_Y+y*TILE+TILE//2
        self.tx,self.ty=self.px,self.py
        self.moving=False; self.speed=3.0*speed_mult
        self.path=[]; self.path_timer=0
        self.detect_range=detect_range
        self.chasing=False; self.direction=1; self.alarm_t=0
    ...
    def update(self,grid,pgx,pgy):
        ...
        if not self.moving and self.path_timer<=0:
            if self.chasing:
                ...
                self.path_timer=6
                if self.alarm_t>1: play_sfx(sfx_alarm,0.15); self.alarm_t=0
            else:
                dirs=[(0,1),(0,-1),(1,0),(-1,0)]; random.shuffle(dirs)
                for ddx,ddy in dirs:
                    nx,ny=self.gx+ddx,self.gy+ddy
                    if 0<=nx<COLS and 0<=ny<ROWS and grid[ny][nx]==0:
                        self.path=[(self.gx,self.gy),(nx,ny)]; self.path_timer=28; break
            if self.path and len(self.path)>1:
                prev=self.gx; nx,ny=self.path[1]
                self.gx,self.gy=nx,ny
                self.tx=MAZE_X+nx*TILE+TILE//2; self.ty=MAZE_Y+ny*TILE+TILE//2
                self.moving=True
        ...
        if self.moving:
            dx=self.tx-self.px; dy=self.ty-self.py; d=math.hypot(dx,dy)
            if d<=self.speed: self.px,self.py=self.tx,self.ty; self.moving=False
            else: self.px+=dx/d*self.speed; self.py+=dy/d*self.speed
    ...
    def draw(self,surf):
        ...
        if guard_img:
            if self.direction==-1: guard_img=pygame.transform.flip(guard_img,True,False)
            if self.chasing:
                red_tint=guard_img.copy()
                red_tint.fill((255,100,100,0),special_flags=pygame.BLEND_RGB_ADD)
                guard_img=red_tint
            surf.blit(guard_img,(cx-guard_img.get_width()//2,cy-guard_img.get_height()//2))
        else:
            # Fallback vector
...
class CCTV:
    def __init__(self,x,y):
        self.gx,self.gy=x,y
        self.px=MAZE_X+x*TILE+TILE//2; self.py=MAZE_Y+y*TILE+TILE//2
        self.angle=random.uniform(-60,60); self.sweep_dir=random.choice([-1,1])
        self.fov=90; self.range=3

    def update(self):
        self.angle+=0.75*self.sweep_dir
        if self.angle>65 or self.angle<-65: self.sweep_dir*=-1

    def in_view(self,pgx,pgy,grid):
        dx=pgx-self.gx; dy=pgy-self.gy; dist=math.hypot(dx,dy)
    ...
    def draw(self,surf):
        ...
        rad=math.radians(self.angle); center=cone_size//2
        pts=[(center,center)]
        ...
        if len(pts)>2: pygame.draw.polygon(cone_surf,(255,60,60,38),pts)
        surf.blit(cone_surf,(cx-center,cy-center))
        # Ikon CCTV PNG
        cctv_img=icons.get("cctv",TILE-6)
        if cctv_img:
            # Rotasi ikon sesuai sudut kamera
            rot_img=pygame.transform.rotate(cctv_img,-self.angle)
            surf.blit(rot_img,(cx-rot_img.get_width()//2,cy-rot_img.get_height()//2))
        else:
...
class Bomb:
    def __init__(self,x,y):
        self.gx,self.gy=x,y
        self.px=MAZE_X+x*TILE+TILE//2; self.py=MAZE_Y+y*TILE+TILE//2
        self.anim_t=random.uniform(0,math.pi*2); self.active=True
    ...
    def draw(self,surf):
        ...
        # Ikon bomb PNG dengan bounce
        bomb_img=icons.get("bomb",TILE-4)
        if bomb_img:
            surf.blit(bomb_img,(cx-bomb_img.get_width()//2,cy-bomb_img.get_height()//2+b))
        else:
...
def setup_level(level:int):
    ...
    # Bombs – validasi dead-end & critical path
    bombs=[]; bomb_pos=set()
...
def draw_pause(surf, fonts):
    ...
    overlay.fill((0,0,0,165)); surf.blit(overlay,(0,0))
    ...
    surf.blit(pause_t,(SCREEN_W//2-pause_t.get_width()//2,box.top+20))
    hint=font_small.render("Tekan P atau ESC untuk lanjut",True,GRAY)
    surf.blit(hint,(SCREEN_W//2-hint.get_width()//2,box.top+88))
    ...
    for idx,(key,icon_key,border,col_n,col_h,label) in enumerate(btn_data):
        br=pygame.Rect(SCREEN_W//2-170, box.top+138+idx*82, 340, 58)
        rects[key]=br
...
def main():
    ...
    state="menu"; level=1; menu_t=0; selected_level=1
    grid,player,guards,cctvs,bombs,cfg=setup_level(level)
    start_time=time.time(); pause_elapsed=0.0
    gameover_reason="YOU GOT CAUGHT!"; no_cctv_hit=True

    # Rect tombol win – disimpan setiap frame render agar click bisa dicek
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if state==STATE_MENU:
                if event.type==pygame.MOUSEBUTTONDOWN:
                    ...
                    if play_r.collidepoint(mx,my):
                        ...
                        grid,player,guards,cctvs,bombs,cfg=setup_level(level)
                        start_time=time.time(); pause_elapsed=0.0
                        no_cctv_hit=True; particles=[]; state=STATE_PLAYING
                    btn_w=110; total_bw=3*btn_w+2*20
                    sx=SCREEN_W//2-total_bw//2
                    for lv in [1,2,3]:
                        lbr=pygame.Rect(sx+(lv-1)*(btn_w+20),318,btn_w,62)
                        if lbr.collidepoint(mx,my): selected_level=lv
            ...
            # ── PLAYING ───────────────────────────────────────────────────────
                ...
                if event.type==pygame.KEYDOWN:
                    if event.key in(pygame.K_ESCAPE,pygame.K_p):
                        ...
                        grid,player,guards,cctvs,bombs,cfg=setup_level(level)
                        start_time=time.time(); pause_elapsed=0.0
                        no_cctv_hit=True; particles=[]

            # ── PAUSE ─────────────────────────────────────────────────────────
            elif state==STATE_PAUSED:
                if event.type==pygame.KEYDOWN and event.key in(pygame.K_ESCAPE,pygame.K_p):
                    start_time=time.time(); state=STATE_PLAYING
                if event.type==pygame.MOUSEBUTTONDOWN:
                    box_top=SCREEN_H//2-195
                    resume_r =pygame.Rect(SCREEN_W//2-170,box_top+138,    340,58)
                    restart_r=pygame.Rect(SCREEN_W//2-170,box_top+138+82, 340,58)
                    quit_r   =pygame.Rect(SCREEN_W//2-170,box_top+138+164,340,58)
            ...
            # ── GAME OVER ─────────────────────────────────────────────────────
                ...
                if event.type==pygame.KEYDOWN and event.key==pygame.K_r:
                    ...
                    no_cctv_hit=True; particles=[]; state=STATE_PLAYING
                if event.type==pygame.MOUSEBUTTONDOWN:
                    box_bot=SCREEN_H//2-205+410   # box.bottom
            ...
            # ── WIN ───────────────────────────────────────────────────────────
                ...
                if event.type==pygame.MOUSEBUTTONDOWN:
                    # Gunakan rect yang disimpan saat render
        ...
        if state==STATE_PLAYING:
            ...
            for cctv in cctvs:
                ...
                if player.invincible==0 and cctv.in_view(player.gx,player.gy,grid):
                    player.lives-=1; player.invincible=90; no_cctv_hit=False
                    spawn_particles(player.px,player.py,RED,18)
                    play_sfx(sfx_alarm,0.5)
                    if player.lives<=0:
                        gameover_reason="KENA CCTV!"; play_sfx(sfx_explosion); state=STATE_GAMEOVER
            ...
            for guard in guards:
                ...
                if math.hypot(guard.px-player.px,guard.py-player.py)<TILE*0.78 \
                   and player.invincible==0:
                    player.lives-=2; player.invincible=120
                    spawn_particles(player.px,player.py,ORANGE,28)
                    if player.lives<=0:
                        gameover_reason="TERTANGKAP SATPAM!"; play_sfx(sfx_explosion); state=STATE_GAMEOVER

            for bomb in bombs:
                if bomb.active and math.hypot(bomb.px-player.px,bomb.py-player.py)<TILE*0.68:
                    bomb.active=False; player.lives=0
                    spawn_particles(player.px,player.py,ORANGE,50)
                    spawn_particles(player.px,player.py,RED,25)
                    play_sfx(sfx_explosion,1.0)
                    gameover_reason="KENA BOM!"; state=STATE_GAMEOVER
        ...
        if state==STATE_MENU:
            ...
            elapsed=(pause_elapsed+(time.time()-start_time)) \
                     if state==STATE_PLAYING else pause_elapsed
            draw_hud(screen,player,level,elapsed,FONTS,cfg["title"])

            if state==STATE_GAMEOVER:
                draw_gameover(screen,FONTS,player.score,gameover_reason)

            elif state==STATE_WIN:
                # Simpan rect ke dict agar event handler bisa pakai
                nr,has_next,mr=draw_win(screen,FONTS,player.score,level,elapsed,no_cctv_hit)
                _win_rects["next"]=nr
